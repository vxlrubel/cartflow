...
const handleBulkAction = async (action) => {
  ...
  if (action === 'pending' || action === 'completed' || action === 'cancelled') {
    await store.bulkStatusUpdate(action)
...
<template>
  <div class="space-y-4">
    <div class="bg-white rounded-lg shadow">
      ...
      >
        <h2 class="text-2xl font-medium text-gray-800">All Orders</h2>
      </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-else-if="store.orders.length === 0">
                <td colspan="10" class="px-4 py-8 text-center text-gray-500">No orders found</td>
              </tr>
              <tr v-for="order in store.orders" :key="order.id" class="hover:bg-gray-50">
                ...
                <td class="px-4 py-4">
                  <div class="flex items-center gap-2">
                    ...
                    <template v-else>
                      <button
                        @click="store.restoreOrder(order.id)"
                        class="text-green-600 hover:text-green-900 text-sm font-medium"
                      >
                        Restore
                      </button>
                      <button
                        @click="store.forceDeleteOrder(order.id)"
                        class="text-red-600 hover:text-red-900 text-sm font-medium"
                      >
                        Delete
                      </button>
