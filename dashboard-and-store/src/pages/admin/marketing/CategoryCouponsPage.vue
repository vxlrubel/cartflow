...
const clearSearch = () => { searchInput.value = ''; store.setSearch('') }
...
watch(() => route.query, () => { store.syncFromQuery(); store.fetchCoupons() }, { deep: true })
</script>
  ...
  <div class="space-y-4">
    <div class="bg-white rounded-lg shadow">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 border-b border-neutral-200 p-6">
        <h2 class="text-2xl font-medium text-gray-800">Category Coupons</h2>
      ...
      <div class="flex flex-wrap gap-2 mb-6 px-6">
        <button v-for="tab in [{ label: 'All', value: 'all' }, { label: 'Trash', value: 'trash' }]" :key="tab.value"
          @click="handleStatusChange(tab.value)"
          :class="['py-1 cursor-pointer text-sm font-medium', currentStatus === tab.value ? 'text-theme-600' : 'text-gray-600 hover:text-theme-400']">
          {{ tab.label }}
        </button>
      </div>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                <th class="px-4 py-3 text-left"><input type="checkbox" :checked="store.allSelected" @change="store.toggleSelectAll" class="rounded border-gray-300" /></th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-else-if="store.coupons.length === 0">
                <td colspan="6" class="px-4 py-8 text-center text-gray-500">No coupons found</td>
              </tr>
              <tr v-for="coupon in store.coupons" :key="coupon.id" class="hover:bg-gray-50">
                <td class="px-4 py-4"><input type="checkbox" :checked="store.selectedIds.includes(coupon.id)" @change="store.toggleSelect(coupon.id)" class="rounded" /></td>
                ...
                <td class="px-4 py-4">
                  <div class="flex items-center gap-2">
                    ...
                    <button v-if="!store.trashed" @click="store.deleteCoupon(coupon.id)" class="text-red-600 text-sm">Trash</button>
                    <button v-else @click="store.restoreCoupon(coupon.id)" class="text-green-600 text-sm">Restore</button>
                  </div>
                </td>
              </tr>
            </tbody>
