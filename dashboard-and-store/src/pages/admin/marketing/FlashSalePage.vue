...
const form = ref({
  name: '', type: 'flash', start_date: '', end_date: '', status: 'inactive',
  rules: [{ rule_type: 'flash_sale', conditions: { discount_percentage: 50, max_quantity: 100 } }],
})
...
onMounted(async () => { store.type = 'flash'; await store.fetchOffers() })
</script>
  ...
  <div class="space-y-4">
    <div class="bg-white rounded-lg shadow">
      <div class="flex justify-between items-center mb-6 border-b border-neutral-200 p-6">
        <h2 class="text-2xl font-medium text-gray-800">Flash Sale</h2>
        <button @click="openCreateModal" class="bg-theme-600 text-white px-3 py-1 rounded text-sm">Add Sale</button>
      </div>
      ...
      <div class="px-6">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-else-if="store.offers.length === 0"><td colspan="5" class="px-4 py-8 text-center">No flash sales found</td></tr>
      ...
      <div v-if="showModal" class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
          ...
          <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6 z-10">
            <h3 class="text-lg font-medium mb-4">{{ editingOffer ? 'Edit' : 'Add' }} Flash Sale</h3>
